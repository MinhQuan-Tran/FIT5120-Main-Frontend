<script lang="ts">
  ...
  export default defineComponent({
    ...
    mounted() {
      ...
      //     variant: ['success', 'normal', 'warning', 'danger'][Math.floor(Math.random() * 4)] as
      //       | 'success'
      //       | 'normal'
      //       | 'warning'
      //       | 'danger',
...
<template>
  <div class="stack" aria-live="polite" aria-relevant="additions removals">
    <PopupNotification
      v-if="store.currentNotice"
      :variant="store.currentNotice?.variant"
      :title="store.currentNotice?.title"
    >
